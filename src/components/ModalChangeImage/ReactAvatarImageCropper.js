...
class AvatarImageCropper extends Component {
    ...
    resetOrientation = (file) => new Promise((resolve, reject) => {
      ...
      const reset = (or) => new Promise((resolve) => {
        if (or === 1) {
          return resolve(file);
        }
    ...
    onDrop = (evt) => {
      ...
      const ifImage = file.type.indexOf('png') >= 0 || file.type.indexOf('jpg') >= 0 || file.type.indexOf('jpeg') >= 0;
      ...
      if (ifImage && file.size <= maxsize) {
        ...
        this.resetOrientation(file).then((file) => {
          ...
          img.onload = () => {
            ...
            let sizeW = this.img2D.ratio >= 1 ? this.avatar2D.height * this.img2D.ratio : this.avatar2D.width;
...
(function (view) {
  ...
  const HTMLCanvasElement = view.HTMLCanvasElement;
    ...
    if (!canvas_proto.toBlob) {
      canvas_proto.toBlob = function (callback, type /* , ...args */) {
        ...
        // no reason to decode a data: URI that's just going to become a data URI again
          ...
          if (is_base64) {
            blob = new Blob([decode_base64(data)], { type });
          } else {
            blob = new Blob([decodeURIComponent(data)], { type });
          }
        }
        callback(blob);
      };
